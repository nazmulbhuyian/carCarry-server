const { postpublishRidessService, getpublishRidessService, getpublishAllRidesService } = require("../services/publishRideServices");
...
exports.getAllRides = async (req, res, next) => {
    try {
            ...
            const result = await getpublishAllRidesService(email);
            if (!result) {
                return res.send('nothing found');
            }
            res.status(200).json({
